...
const HomeScreen = () => {
  ...
  const fetchMatches = async () => {
    try {
      ...
      const filteredMatces = matches?.filter(
        match => !dislikedProfiles.some(disliked => disliked.userId == match.userId)
      )
      setUsers(filteredMatces)
    } catch (error) {
